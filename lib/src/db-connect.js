...
const initDatabase = (_appConfig, logger) => models_1.MongoRun.updateMany({ [`config.${models_1.mongoWorkflowsRepositoryKey}`]: { $exists: false } }, { $set: { config: { workflows_repository: '.github' } } }, { new: true, multi: true }, (err, numberAffected) => {
